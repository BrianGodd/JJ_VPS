...
{
    public class CameraPlatformCustom_AR : MonoBehaviour, IPlatformSupport
    {
        //custom
        ...
        public async Task<IPlatformConfigureResult> ConfigurePlatform(IPlatformConfiguration configuration)
        {
            ...
            if (Main_Camera != null) m_CameraTransform = Main_Camera.transform;
        ...
        private async Task<(bool, CameraData)> GetCameraData(CameraDataFormat cameraDataFormat)
        {
            if (!GetIntrinsics(out Vector4 intrinsics))
            {
                ...
                DebugMessage.text = "Could not acquire camera intrinsics.";
                return (false, null);
            }
            ...
            if (m_CameraTransform == null)
            {
                ...
                DebugMessage.text = "Could not acquire camera pose.";
                return (false, null);
            }
            ...
            //     DebugMessage.text = "Could not acquire camera image.";
            //     return (false, null);
            // }
            ...
            camBytes = data.GetBytes();

            texture.LoadRawTextureData(camBytes);
        ...
        // private async Task<CameraData> JJDataToCamera(CameraData data)
        // {
        //     int pixelCount = width * height;
        //     byte[] rgbData = new byte[pixelCount * 3];
        ...
        //     CameraData newData = data.Copy(new SimpleImageData(rgbData));
    ...
    public class ARFImageData : ImageData
    {
        ...
        public byte[] CopyBytes()
        {
            ...
            //byte[] rgbData = ConvertRGBA32ToRGB24(CamRenderer_ARF.camBytes, Image.width, Image.height);
            //return rgbData;
            return bytes;
        }

        private byte[] ConvertRGBA32ToRGB24(byte[] rgbaData, int width, int height)
        {
            int pixelCount = width * height;
            byte[] rgbData = new byte[pixelCount * 3];
