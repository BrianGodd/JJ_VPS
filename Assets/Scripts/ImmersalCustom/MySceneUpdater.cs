...
namespace Immersal.XR
{
    ...
    public class MySceneUpdater : MonoBehaviour, ISceneUpdater
    {
        public TextMeshProUGUI LocalizeInfoText;
	    ...
	    public async Task UpdateScene(MapEntry entry, ICameraData cameraData, ILocalizationResult localizationResult)
	    {
            ...
            //show info outside
            LocalizeInfoText.text = $"Pos: {localizedPos:F3}\nRot: {localizedRot.eulerAngles:F3}";
			RAGMaster.pos_x = localizedPos.x;
			RAGMaster.pos_z = localizedPos.z;
			RAGMaster.rot = localizedRot.eulerAngles.y;
